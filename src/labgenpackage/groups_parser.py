...
def pars_groups_file()->list[dict[str, list[Group]], str, list[str]]:
            ...
            for line in file:
                ...
                if(len(split_line)==5):
                    if split_line[1] in days and re.fullmatch('[0-9][0-9]:[0-9][0-9]-[0-9][0-9]:[0-9][0-9]', split_line[2]) and re.fullmatch('[a-cA-C][1-9][0-9][0-9]', split_line[3]) and re.fullmatch('1?[0-9][0-9]?', split_line[4]):
                        ...
                        group_errors.append(f'{line}')
                        logger.warning(f'Potential error with provided group in line {line}!')
        if(len(groups)==0):
            logger.error(f'Failed to finde groups in {fpath}')
            raise ValueError
        return [groups,file_name,group_errors]
